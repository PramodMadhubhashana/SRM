...
class AddNonAcdamicStarff extends StatefulWidget {
  final String id;
  const AddNonAcdamicStarff({super.key, required this.id});

  @override
  State<AddNonAcdamicStarff> createState() => _AddNonAcdamicStarffState();
}

class _AddNonAcdamicStarffState extends State<AddNonAcdamicStarff> {
  ...
  Future<void> _addnonAcadamicStaff() async {
    ...
    if (url != 'Fail') {
      String result = await _authService.addData(
          stId, fNme, lNme, email, address, pNo, psd, "Non-Staff", url);
  ...
  Future<String> uploadImge(dynamic file) async {
    if (file != null) {
      try {
        String url = await _authService.addNonAcadsmicStaffImage(file);
        return url;
      } catch (e) {
        return 'Fail';
      }
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ElevatedButton(
                            onPressed: _addnonAcadamicStaff,
                            child: const Text(
                              "Add",
                              style: TextStyle(
